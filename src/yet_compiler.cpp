...
void yetexception(string message, string filename = ""){
    cout << message << "\tfile:" << filename << endl;
    exit(1);
}
void yetexception(string message, Token t, string filename = ""){
    cout << message << " in line " << t.getlineno() << "\tfile:" << filename << endl;
    exit(1);
}
class Lexer{
private:
...
void Lexer::readline(){
    ...
    while(i < endpos){
        ...
        if('0'<=line[i] && line[i]<='9' || ('-'==line[i] && ('0'<=line[i + 1] && line[i + 1]<='9'))){
            ...
            while(!('0'<=line[i] && line[i]<='9') && !('a'<=line[i] && line[i]<='z' || 'A'<=line[i] && line[i]<='Z' || line[i] == '_') && !(' '==line[i] || '\n'==line[i] || '\t'==line[i]) && (*find(operators.begin(), operators.end(), str) != str || *find(operators.begin(), operators.end(), str + line[i]) == str + line[i])){
                str += line[i++];
            }
            token = IdToken(lineno, str);
        }
...
int main(int argc, char* argv[]){
	if(argc == 5){
		...
		int compile_type = atoi(argv[3]);
		bool is_print = atoi(argv[4]);
		//cout << in_file_name << " " << out_file_name << " " << compile_type << " " << is_print << endl;
		...
		yetexception("no matching argments");
		return 1;
	}
}
